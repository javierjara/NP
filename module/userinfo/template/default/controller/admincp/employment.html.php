...
{$sCreateJs}
<form method="post" action="{url link="admincp.userinfo.employment"}" onsubmit="return startProcess({$sGetJsForm}, false);" name="empform" id="empform">
...
<form method="post" action="{url link="admincp.userinfo.employment"}">	
<div class="table_header">
	{phrase var='userinfo.manage_employment_fields'}
</div>
	<table>
	<tr>
	...
	<tr id="js_row{$aField.id}" class="checkRow{if is_int($iKey/2)} tr{else}{/if}">
		<td style="vertical-align:middle;"><input type="checkbox" name="id[]" class="checkbox" value="{$aField.id}|{$aField.name}" id="js_id_row{$aField.id}" {if $aField.name == 'employer' || $aField.name == 'date' || $aField.name == 'position'}disabled="disabled"{/if} /></td>
		<td style="vertical-align:middle;">
				{$aField.ucname|clean}
		</td>
		<td style="vertical-align:middle;">{if $aField.type == 0}{phrase var='userinfo.date'}{elseif $aField.type == 1}{phrase var='userinfo.text_field'}{elseif $aField.type == 2}{phrase var='userinfo.text_box'}{/if}<br />{phrase var='userinfo.language_phrase'}: <span style="font-style:italic">userinfo.field_{$aField.name}</span></td>
		<td class="t_center" style="vertical-align:middle;">{if $aField.id != 1 && $aField.id != 2}<div id="form_container"><div id="left_forms" style="float:left">{if {$iKey != 0 && $iKey != 1 && $iKey != 2}<div id="form1" style="float:left;"><form action="{url link='admincp.userinfo.employment'}" method="post"><input type="hidden" name="field_id" value="{$aField.id}" /><input class="button" id="fldup" name="fldup" type="submit" value="{phrase var='userinfo.up'}" style="margin-left:0;" /></form></div>{/if}{if {$aField.position < count($aFields)}<div id="form2" style="float:left;"><form action="{url link='admincp.userinfo.employment'}" method="post"><input type="hidden" name="field_id" value="{$aField.id}" /><input class="button" id="flddown" name="flddown" type="submit" value="{phrase var='userinfo.down'}" {if {$iKey != 0 && $iKey != 1 && $iKey != 2}style="margin-left:5px;"{/if} /></form></div>{/if}</div>{/if}</td>
		<td class="t_center" style="vertical-align:middle;">
			{if $aField.name == 'employer' || $aField.name == 'position' || $aField.name == 'date'}
		...
		<td class="t_center" style="vertical-align:middle;">
			{if $aField.name == 'employer' || $aField.name == 'date'}
				{phrase var='userinfo.yes'}
			{else}
