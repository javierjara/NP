 ...
 * @version 		$Id: ajax.class.php 6623 2013-09-11 13:58:32Z Fern $
 */
    ...
    public function process()
    {
		...
		foreach ($aImages as $iKey => $aImage)
		{
		    ...
		    if ($aImage['completed'] == 'false')
		    {
				...
				if (isset($aPhoto['photo_id']))
				{
				    ...
				    if (((Phpfox::getParam('core.allow_cdn') && (Phpfox::getParam('photo.delete_original_after_resize') || !Phpfox::getParam('core.keep_files_in_server'))) && $this->get('is_page') != 1) && !defined('PHPFOX_IS_HOSTED_SCRIPT'))
				    {
		...
		if ($iNotCompleted === 0)
		{
			...
			if (!Phpfox::getUserParam('photo.photo_must_be_approved') && !$this->get('is_cover_photo'))
			{
				...
				{
					$iFeedId = Phpfox::getService('feed.process')->callback($aCallback)->add('photo', $aPhoto['photo_id'], $aPhoto['privacy'], $aPhoto['privacy_comment'], (int) $this->get('parent_user_id', 0), null, 0, 0, null, $_POST['np_post_type'],$_POST['np_post_category']);
				}
		    ...
		    else if ($this->get('action') == 'upload_photo_via_share')
		    {
				...
				{
					Phpfox::getService('feed')->callback($aCallback)->processAjax($iFeedId, $_POST['np_post_type'],$_POST['np_post_category']);
