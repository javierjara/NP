 ...
 * @version 		$Id: frame.class.php 6653 2013-09-16 12:28:51Z Raymond_Benc $
 */
	...
	public function process()
	{
		...
		if (count($aImages))
		{
			...
			else 
			{
				...
				echo '$.ajaxCall(\'photo.process\', \''. ((isset($aVals['page_id']) && !empty($aVals['page_id'])) ? 'is_page=1&' : '') .'js_disable_ajax_restart=true' . $sExtra . '&twitter_connection=' . ((isset($aVals['connection']) && isset($aVals['connection']['twitter'])) ? $aVals['connection']['twitter'] : '0') . '&facebook_connection=' . (isset($aVals['connection']['facebook']) ? $aVals['connection']['facebook'] : '0') . '&custom_pages_post_as_page=' . $this->request()->get('custom_pages_post_as_page') . '&photos=' . urlencode(json_encode($aImages)) . '&action=' . $sAction . '' . (isset($iFeedId) ? '&feed_id=' . $iFeedId : '') . '' . ($aCallback !== null ? '&callback_module=' . $aCallback['module'] . '&callback_item_id=' . $aCallback['item_id'] : '') . '&parent_user_id=' . (isset($aVals['parent_user_id']) ? (int) $aVals['parent_user_id'] : 0) . '&is_cover_photo=' . (isset($aVals['is_cover_photo']) ? '1' : '0') . ((isset($aVals['page_id']) && $aVals['page_id'] > 0) ? '&page_id='.$aVals['page_id'] : '') . '&np_post_type=' . $aVals['np_post_type'] . '&np_post_category=' . $aVals['np_post_category'] . '&np_checkin_name=' . $aVals['np_checkin_name']. '&np_checkin_lat=' . $aVals['np_checkin_lat']. '&np_checkin_lng=' . $aVals['np_checkin_lng'] . '&np_youtube=' . $aVals['np_youtube'] . '&np_youtube_title=' . $aVals['np_youtube_title'] . '&np_youtube_desc=' . $aVals['np_youtube_desc']. '&np_youtube_thumb=' . $aVals['np_youtube_thumb'] .'\');';
