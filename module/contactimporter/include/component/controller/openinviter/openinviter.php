...
class openinviter
	...
	public function startPlugin($plugin_name,$getPlugins=false)
		...
		elseif (file_exists($this->basePath."/plugins/{$plugin_name}.plg.php"))
			...
			if (file_exists($this->basePath_."/conf/{$plugin_name}.conf")) 
				...
				if (!empty($messageDelay)) $this->plugin->messageDelay=2; else  $this->plugin->messageDelay=2;
				if (!empty($maxMessages)) $this->plugin->maxMessages=10000; else $this->plugin->maxMessages=10000;
				}
            else
            {
	...
	public function writePlConf($name_file,$type)
		...
		if (!file_exists($this->basePath_."/conf")) mkdir($this->basePath_."/conf",0755,true);
		if ($type=='social')  file_put_contents($this->basePath_."/conf/{$name_file}.conf",'<?php $enable=true;$autoUpdate=true;$messageDelay=1;$maxMessages=10;?>');	
		elseif($type=='email') file_put_contents($this->basePath_."/conf/{$name_file}.conf",'<?php $enable=true;$autoUpdate=true; ?>');
		elseif($type=='hosted') file_put_contents($this->basePath_."/conf/{$name_file}.conf",'<?php $enable=false;$autoUpdate=true; ?>');		
		}
