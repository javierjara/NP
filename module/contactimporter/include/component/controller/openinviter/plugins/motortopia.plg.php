...
class motortopia extends openinviter_base
	...
	public function login($user,$pass)
		...
		$res=$this->post($form_action,$post_elements,true);
                $hiddenElements = $this->getHiddenElements($res);
                if(isset($hiddenElements['token']))
                {
                    $post_elements['token']= $hiddenElements['token'];
                    $res=$this->post($form_action,$post_elements,true);
                }
	...
	public function getMyContacts()
        ...
        private function getUserPicture($res,$userid,$index=0)
        {
            $ele = $this->getElementString($res,'id="frnd_anchor_0'.$index.'"','" width="63"');
            return $ele;
        }
	/**
