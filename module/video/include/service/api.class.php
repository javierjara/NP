...
class Video_Service_Api extends Phpfox_Service 
{
	...
	public function get($iLinkId = 0)
	{
		...
		*/
		...
		{
			$this->database()->select('lik.like_id AS is_liked, ')->leftJoin(Phpfox::getT('like'), 'lik', 'lik.type_id = \'video\' AND lik.item_id = p.video_id AND lik.user_id = ' . Phpfox::getUserId());
		}		
